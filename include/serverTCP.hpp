...
namespace bh {
    ...
    public:
    /**
     * @brief Constructs a TCP server and binds to the specified port.
     *
     * Convenience constructor that creates the underlying TCP socket,
     * binds it to INADDR_ANY on the given port and sets it to listen.
     *
     ...
     *
     * Creates a ServerTCP object without immediately creating or binding
     * the underlying socket. After constructing with the default
     * constructor you must call bind() and listen() before calling run().
     */
    ServerTCP();
         ...
         * specific server logic (e.g., daytime, echo).
         */
        virtual void run() {} 
